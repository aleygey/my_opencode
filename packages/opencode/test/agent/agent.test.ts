...
test("defaultAgent returns orchestrator when build is disabled and default_agent not set", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        build: { disable: true },
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const agent = await load(tmp.path, (svc) => svc.defaultAgent())
      // orchestrator ships as the first primary agent in this fork, so it still wins even when build is disabled
      expect(agent).toBe("orchestrator")
    },
  })
})
